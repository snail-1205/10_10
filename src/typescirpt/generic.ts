...
customLog<number>(1, 2);

customLog<string>("g", "d");

// customLog<string>("1",[1]) > "1"과 [1]은 타입이 다르므로 error!

// customLog<string>("1", 2) // 역시 에러를 띄움. 타입이 달라용

customLog(1, 2); // 이런식으로 해도 ts는 똑똑해서 꺽쇠에 number가 들어갈 걸 알고있음
...
customLog2<number, string>(32, "ㅎㅇ");

customLog2<number, number>(32, 10);
...
customLog3(3, "가나다") // 문자열은 length라는 멤버가 있기 때문에 ㄱㅊ

customLog3(3, [1,2,3,4,]) // 배열은 length라는 멤버가 있기 때문에 ㄱㅊ

//직접 만들진 않지만 class에도 제너릭 타입을 선언할 수가 있음
class Animal<T> {
